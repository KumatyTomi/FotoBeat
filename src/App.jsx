...
export default function App() {
    ...
    nextAssets.forEach((asset) => {
      ...
      image.onload = () => {
        ...
        setMediaAssets((current) => current.map((item) => (
          item.id === asset.id
            ? { ...item, status: 'ready', width: image.naturalWidth, height: image.naturalHeight, orientation: getOrientation(image.naturalWidth, image.naturalHeight), image }
            : item
        )));
      };
      image.onerror = () => {
        if (cancelled) return;
        setMediaAssets((current) => current.map((item) => (item.id === asset.id ? { ...item, status: 'error' } : item)));
      };
      image.src = asset.url;
    });
  ...
  useEffect(() => {
    ...
    setPinnedAssetsByClip((current) => {
      const next = Object.fromEntries(Object.entries(current).filter(([, assetId]) => activeIds.has(assetId)));
      return Object.keys(next).length === Object.keys(current).length ? current : next;
    });
  }, [selectedMediaAssets]);

  const timeline = useMemo(() => buildDraftTimeline({
    images: selectedMediaAssets,
    audio,
    format,
    preset,
    audioAnalysis,
    clipDurationScale
  }), [selectedMediaAssets, audio, format, preset, audioAnalysis, clipDurationScale]);
  ...
  function pinAssetToCurrentClip(assetId) {
    setPinnedAssetsByClip((current) => ({ ...current, [previewPlayback.clipIndex]: assetId }));
  }
  ...
  async function importProjectFile(event) {
    ...
    try {
      ...
      setProject({ ...DEFAULT_PROJECT, ...imported.project, importedAt: new Date().toISOString() });
      if (remapped.selectedOrder.length) setSelectedAssetIds(remapped.selectedOrder);
      setPinnedAssetsByClip(remapped.pinnedAssetsByClip);
      setProjectIoStatus({ type: 'success', message: `Zaimportowano projekt. Dopasowano ${remapped.selectedOrder.length}/${imported.media?.selectedImages?.length ?? 0} mediów.` });
    } catch (error) {
    ...
    <main className="app-shell">
      ...
      <section id="project" className="project-panel">
        <div className="section-heading project-heading">
          <div><p className="panel-kicker">Projekt</p><h2>Autosave, snapshoty, import i eksport</h2><p>Stan projektu zapisuje się automatycznie. Możesz pobrać `.fotobeat.json`, skopiować JSON lub zaimportować projekt.</p></div>
      ...
      <section id="preview" className="render-preview-panel">
        <div className="section-heading project-heading">
          ...
          <div className="preview-hud"><span>{selectedFormat.width}×{selectedFormat.height}</span><span>{previewPlayback.time}s</span><span>Klip {previewPlayback.clipIndex}/{timeline.clips.length}</span><span>{selectedMediaAssets.length} kadrów</span><span>Tempo ×{clipDurationScale}</span></div>
        </div>
      ...
      <section className="media-panel">
        <div className="section-heading project-heading">
          ...
          <div className="preview-hud"><span>{mediaAssets.length} plików</span><span>{selectedMediaAssets.length} aktywnych</span><span>{Object.keys(pinnedAssetsByClip).length} przypięć</span>{mediaAssets.length > 0 && <button className="ghost-button compact" onClick={selectAllMedia}>Zaznacz wszystko</button>}{pinnedAssetsByClip[previewPlayback.clipIndex] && <button className="ghost-button compact" onClick={() => clearPinnedClip(previewPlayback.clipIndex)}>Odepnij klip {previewPlayback.clipIndex}</button>}</div>
        </div>
        {mediaAssets.length === 0 ? <span className="empty-state">Wrzuć zdjęcia, a tutaj pojawią się miniatury, scoring, orientacja i kontrola timeline.</span> : (
          <div className="media-grid">{scoredMediaAssets.map((asset) => {
            ...
            return <article key={asset.id} className={selected ? 'media-card selected' : 'media-card'}>
              <button className="media-thumb-button" onClick={() => toggleMediaAsset(asset.id)}><span className="media-thumb"><img src={asset.url} alt={asset.name} />{selected && <i><CheckCircle2 size={18} /></i>}</span></button>
              ...
              <div className="media-actions"><button onClick={() => moveMediaAsset(asset.id, -1)} disabled={!selected || activeIndex <= 0}>↑</button><button onClick={() => moveMediaAsset(asset.id, 1)} disabled={!selected || activeIndex === selectedAssetIds.length - 1}>↓</button><button onClick={() => pinAssetToCurrentClip(asset.id)} disabled={!selected}>Przypnij</button></div>
            </article>;
          })}</div>
        )}
      </section>
...
function WaveformPreview({ analysis, durationScale }) {
  ...
  return <div className="waveform-box">
    <div className="waveform-bars">{waveform.map((value, index) => <span key={`wf-${index}`} style={{ height: `${Math.max(8, value * 100)}%` }} />)}</div>
    <div className="beat-grid">{beats.slice(0, 48).map((beat, index) => <i key={`beat-${index}`} style={{ left: `${Math.min(100, ((beat.time * durationScale) / (duration * durationScale)) * 100)}%`, opacity: 0.35 + beat.energy * 0.55 }} />)}</div>
  </div>;
}
...
function buildProjectExportPayload({ project, timeline, mediaAssets, selectedMediaAssets, selectedAssetIds, pinnedAssetsByClip, selectedFormat, audio }) {
  return {
    ...
    media: {
      ...
      pinnedAssetsByClip,
      selectedImages: selectedMediaAssets.map((asset) => ({ id: asset.id, name: asset.name, size: asset.size, width: asset.width, height: asset.height, orientation: asset.orientation, status: asset.status, score: scoreMediaAsset(asset, selectedFormat) })),
...
function parseProjectFile(text) {
  ...
  if (parsed.schema !== 'fotobeat.project.v1' || !parsed.project) throw new Error('To nie jest poprawny plik projektu FotoBeat.');
  return parsed;
}
...
function remapImportedMedia(importedMedia = {}, currentMediaAssets) {
  ...
  const selectedOrder = uniqueIds(importedMedia.selectedOrder ?? []).map((assetId) => assetsById.get(assetId) ?? assetsByName.get(importedById.get(assetId)?.name)).filter(Boolean).map((asset) => asset.id);
  const fallbackOrder = (importedMedia.selectedImages ?? []).map((asset) => assetsById.get(asset.id) ?? assetsByName.get(asset.name)).filter(Boolean).map((asset) => asset.id);
  ...
  Object.entries(importedMedia.pinnedAssetsByClip ?? {}).forEach(([clipNumber, importedAssetId]) => {
    ...
    if (matchedAsset) pinnedAssetsByClip[clipNumber] = matchedAsset.id;
  });
  return { selectedOrder: finalOrder, pinnedAssetsByClip };
}

function uniqueIds(ids) { return [...new Set(ids.filter(Boolean))]; }
function safeFilename(value) { return (value || 'fotobeat-project').toLowerCase().replace(/[^a-z0-9ąćęłńóśźż_-]+/gi, '-').replace(/^-+|-+$/g, ''); }
function buildMediaId(file, index) { return `${file.name}-${file.size}-${file.lastModified}-${index}`; }
function getOrientation(width, height) { if (!width || !height) return 'unknown'; if (Math.abs(width - height) < Math.max(width, height) * 0.05) return 'square'; return width > height ? 'landscape' : 'portrait'; }
function scoreMediaAsset(asset, selectedFormat) { let score = 0; if (asset.status === 'ready') score += 25; if (asset.width >= 1080 && asset.height >= 1080) score += 25; if (asset.width >= selectedFormat.width * 0.55 || asset.height >= selectedFormat.height * 0.55) score += 15; if (isOrientationFit(asset.orientation, selectedFormat.id)) score += 25; if (asset.size > 120000) score += 10; return Math.min(100, score); }
function isOrientationFit(orientation, formatId) { if (formatId === 'vertical') return orientation === 'portrait'; if (formatId === 'wide') return orientation === 'landscape'; if (formatId === 'square') return orientation === 'square' || orientation === 'portrait'; return false; }
function resolveMediaForClip(clipIndex, selectedMediaAssets, pinnedAssetsByClip) { const pinnedAssetId = pinnedAssetsByClip[clipIndex]; const pinnedAsset = selectedMediaAssets.find((asset) => asset.id === pinnedAssetId); if (pinnedAsset) return pinnedAsset; return selectedMediaAssets[(clipIndex - 1) % Math.max(selectedMediaAssets.length, 1)]; }
function getPinnedLabel(assetId, pinnedAssetsByClip) { const clipNumbers = Object.entries(pinnedAssetsByClip).filter(([, pinnedAssetId]) => pinnedAssetId === assetId).map(([clipNumber]) => clipNumber); return clipNumbers.length ? `Klip ${clipNumbers.join(', ')}` : ''; }
...
async function analyzeAudioFile(file) {
  ...
  for (let offset = 0; offset < channel.length; offset += windowSize) {
    ...
    for (let index = offset; index < end; index += 1) sum += Math.abs(channel[index]);
  ...
  context.close?.();
  return { status: 'ready', fileName: file.name, duration: Number(duration.toFixed(2)), bpm: estimatedBpm, energy: Number(normalizedEnergy.toFixed(2)), averageBeatStep: Number((60 / estimatedBpm).toFixed(2)), waveform, beats };
}

function buildWaveform(channel, buckets) {
...
function buildFallbackWaveform(count) { return Array.from({ length: count }).map((_, index) => Number((0.25 + Math.abs(Math.sin(index * 0.38)) * 0.65).toFixed(3))); }
function buildFallbackBeats(bpm, count) { const step = 60 / bpm; return Array.from({ length: count }).map((_, index) => ({ time: Number((index * step).toFixed(2)), energy: Number((0.5 + ((index % 4) * 0.12)).toFixed(2)) })); }
function describeAudioAnalysis(audioAnalysis) { if (audioAnalysis.status === 'analyzing') return `Analizuję plik ${audioAnalysis.fileName} i przygotowuję siatkę cięć.`; if (audioAnalysis.status === 'fallback') return `Używam awaryjnej beat mapy dla ${audioAnalysis.fileName}.`; return `Plik ${audioAnalysis.fileName}: długość ${audioAnalysis.duration}s, szacunkowe BPM ${audioAnalysis.bpm}.`; }
function findClipAtTime(clips, time) { return [...clips].reverse().find((clip) => time >= clip.start) ?? clips[0]; }
...
function drawRenderPreview(canvas, { time, clip, clipIndex, totalClips, format, preset, imageCount, mediaAsset, projectName }) {
  ...
  background.addColorStop(0, '#05050a'); background.addColorStop(0.45, colors.deep); background.addColorStop(1, '#10101f');
  ...
  ctx.save(); ctx.translate(width / 2, height / 2); ctx.rotate(getPresetRotation(preset.id, progress, energy)); ctx.scale(1 + pulse * 0.035 + energy * 0.018, 1 + pulse * 0.035 + energy * 0.018); drawFrameStack(ctx, width, height, colors, clipIndex, imageCount, mediaAsset); ctx.restore();
  drawScanlines(ctx, width, height, preset.id, progress); drawPreviewText(ctx, width, height, { projectName, clip, clipIndex, totalClips, preset, format, colors, mediaAsset });
}
function getClipProgress(clip, time) { if (!clip) return 0; return Math.min(1, Math.max(0, (time - clip.start) / Math.max(clip.duration, 0.1))); }
function getPresetColors(presetId) { const map = { neonPulse: { primary: '#00d3ff', secondary: '#7c3cff', deep: '#10103a' }, smokeCut: { primary: '#c8d0ff', secondary: '#6d7188', deep: '#171927' }, matrixGlitch: { primary: '#6cff8d', secondary: '#00d3ff', deep: '#061b13' }, sinCity: { primary: '#ff3b5c', secondary: '#f4f4f4', deep: '#17080c' }, spiralZoom: { primary: '#ffb86b', secondary: '#7c3cff', deep: '#1c102f' }, dreamFade: { primary: '#ffd6f2', secondary: '#8be9ff', deep: '#171429' } }; return map[presetId] ?? map.neonPulse; }
function getPresetRotation(presetId, progress, energy) { if (presetId === 'spiralZoom') return progress * 0.16; if (presetId === 'matrixGlitch') return Math.sin(progress * 24) * 0.006 * energy; if (presetId === 'dreamFade') return Math.sin(progress * Math.PI) * 0.01; return Math.sin(progress * Math.PI * 2) * 0.018 * energy; }
function drawGlow(ctx, x, y, radius, color, alpha) { const gradient = ctx.createRadialGradient(x, y, 0, x, y, radius); gradient.addColorStop(0, hexToRgba(color, alpha)); gradient.addColorStop(1, 'rgba(0, 0, 0, 0)'); ctx.fillStyle = gradient; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); }
function drawFrameStack(ctx, width, height, colors, clipIndex, imageCount, mediaAsset) { const baseWidth = width * 0.58; const baseHeight = height * 0.58; for (let index = 4; index >= 0; index -= 1) { const offset = (index - 2) * width * 0.025; const alpha = 0.18 + (5 - index) * 0.08; ctx.fillStyle = index % 2 === 0 ? hexToRgba(colors.primary, alpha) : hexToRgba(colors.secondary, alpha); ctx.strokeStyle = hexToRgba('#ffffff', 0.16 + alpha * 0.22); ctx.lineWidth = Math.max(2, width * 0.002); roundRect(ctx, -baseWidth / 2 + offset, -baseHeight / 2, baseWidth, baseHeight, width * 0.025); ctx.fill(); ctx.stroke(); } const heroWidth = baseWidth * 0.92; const heroHeight = baseHeight * 0.92; const heroX = -heroWidth / 2; const heroY = -heroHeight / 2; if (mediaAsset?.status === 'ready' && mediaAsset.image) { drawImageCover(ctx, mediaAsset.image, heroX, heroY, heroWidth, heroHeight, width * 0.022); ctx.fillStyle = hexToRgba(colors.deep, 0.18); roundRect(ctx, heroX, heroY, heroWidth, heroHeight, width * 0.022); ctx.fill(); } else { ctx.fillStyle = 'rgba(255, 255, 255, 0.88)'; ctx.font = `700 ${Math.max(26, width * 0.028)}px Inter, sans-serif`; ctx.textAlign = 'center'; ctx.fillText(imageCount ? `PHOTO ${clipIndex}` : 'DROP PHOTOS', 0, 0); } }
function drawImageCover(ctx, image, x, y, width, height, radius) { const ratio = Math.max(width / image.naturalWidth, height / image.naturalHeight); const drawWidth = image.naturalWidth * ratio; const drawHeight = image.naturalHeight * ratio; const drawX = x + (width - drawWidth) / 2; const drawY = y + (height - drawHeight) / 2; ctx.save(); roundRect(ctx, x, y, width, height, radius); ctx.clip(); ctx.drawImage(image, drawX, drawY, drawWidth, drawHeight); ctx.restore(); }
function drawScanlines(ctx, width, height, presetId, progress) { if (presetId !== 'matrixGlitch' && presetId !== 'sinCity') return; ctx.save(); ctx.globalAlpha = presetId === 'matrixGlitch' ? 0.18 : 0.08; ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 1; const gap = Math.max(12, height * 0.012); const shift = progress * gap * 2; for (let y = -gap; y < height + gap; y += gap) { ctx.beginPath(); ctx.moveTo(0, y + shift); ctx.lineTo(width, y + shift); ctx.stroke(); } ctx.restore(); }
function drawPreviewText(ctx, width, height, { projectName, clip, clipIndex, totalClips, preset, format, colors, mediaAsset }) { const margin = width * 0.06; const bottom = height - margin; ctx.fillStyle = 'rgba(255, 255, 255, 0.92)'; ctx.font = `800 ${Math.max(24, width * 0.025)}px Inter, sans-serif`; ctx.textAlign = 'left'; ctx.fillText(projectName || 'FotoBeat Project', margin, margin * 1.25); ctx.fillStyle = hexToRgba(colors.primary, 0.92); ctx.font = `900 ${Math.max(16, width * 0.014)}px Inter, sans-serif`; ctx.fillText(`${preset.name} · ${format.label}`, margin, margin * 1.75); ctx.fillStyle = 'rgba(255, 255, 255, 0.78)'; ctx.font = `700 ${Math.max(18, width * 0.017)}px Inter, sans-serif`; ctx.fillText(`Clip ${clipIndex}/${totalClips} · ${clip?.section ?? 'intro'} · ${clip?.effect ?? 'fade'}`, margin, bottom); if (mediaAsset?.name) { ctx.font = `500 ${Math.max(15, width * 0.013)}px Inter, sans-serif`; ctx.fillStyle = 'rgba(255, 255, 255, 0.58)'; ctx.fillText(mediaAsset.name.slice(0, 42), margin, bottom - margin * 0.42); } ctx.textAlign = 'right'; ctx.fillText('FotoBeat.me', width - margin, bottom); }
...
function hexToRgba(hex, alpha) { const normalized = hex.replace('#', ''); const value = Number.parseInt(normalized, 16); return `rgba(${(value >> 16) & 255}, ${(value >> 8) & 255}, ${value & 255}, ${alpha})`; }
